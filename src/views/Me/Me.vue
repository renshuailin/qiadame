<template>
  <div class="me">
    <div class="header-info">
      <div class="header-img"></div>
      <div class="header-profile">
        <p class="id" v-if="userInfo">{{userInfo._id}}</p>
        <p class="id" v-else @click="login">登录/注册</p>
        <p class="phone">
          <i class="fa fa-mobile"></i>
          <span v-if="userInfo">{{hidePhone(userInfo.phone) }}</span>
          <span v-else>登录后享受更多特权</span>
        </p>
      </div>
      <i class="fa fa-angle-right"></i>
    </div>
    <div v-if="userInfo">
      <div class="address">
        <i class="fa fa-map-marker"></i>
        <div class="address-index" @click="address">
          <span>我的地址</span>
          <i class="fa fa-angle-right"></i>
        </div>
      </div>
      <button @click="out" class="btn-out">退出登录</button>
    </div>
  </div>
</template>
<script>
export default {
  name: "Me",
  data() {
    return {
      userInfo: "",
    };
  },
  beforeRouteEnter(to, from, next) {
    // ...
    next((vm) => vm.getData());
  },
  methods: {
    login() {
      this.$router.push("/login");
    },

    getData() {
      const user_id = localStorage.ele_login;
      this.$axios(`/api/user/user_info/${user_id}`).then((res) => {
        console.log(res.data);
        this.userInfo = res.data;
      });
    },
    hidePhone(phone) {
      return phone.replace(/(\d{3})\d{4}(\d{4})/, "$1****$2");
    },
    out() {
      localStorage.removeItem("ele_login");
      this.$router.push("/login");
    },
    address() {
      if (this.userInfo.myAddress.length > 0) {
        this.$router.push("/myaddress");
      } else {
        this.$router.push("/addaddress");
      }
    },
  },
};
</script>
<style scoped>
/* 登录注册 */
.me {
  width: 100%;
  height: 100%;
  overflow: auto;
  box-sizing: border-box;
}
.header-info {
  display: flex;
  background-image: linear-gradient(90deg, #38c27e, #38c27e);
  padding: 7vw 4vw;
  color: #fff;
  align-items: center;
}
.header-img {
  width: 60px;
  height: 60px;
  border-radius: 50%;
  background-position: 0px 0px;
  background-size: 60px;
  background-image: url(https://shadow.elemecdn.com/faas/h5/static/sprite.3ffb5d8.png);
}
.header-profile {
  overflow: hidden;
  margin-left: 5vw;
  flex-grow: 1;
}
.header-profile .id {
  max-width: 40vw;
  overflow: hidden;
  white-space: nowrap;
  text-overflow: ellipsis;
  font-size: 1rem;
  margin-bottom: 2vw;
  font-weight: 700;
  display: flex;
  align-items: center;
}
.header-profile .phone {
  display: flex;
  align-items: center;
  font-size: 1rem;
}
.phone > i {
  margin-right: 0.5vw;
  font-size: 1rem;
}
.headInfo > i {
  font-size: 1.2rem;
}
/* 地址 */
.address {
  margin-top: 2.666667vw;
  border-top: 1px solid #ddd;
  border-bottom: 1px solid #ddd;
  font-size: 1rem;
  line-height: 4.533333vw;
  background: #ffffff;
  display: flex;
  align-items: center;
  padding-left: 6.666667vw;
  color: #333;
}
.address > i {
  font-size: 1.3rem;
  color: #38c27e;
  margin-right: 2.666667vw;
}
.address-index {
  display: flex;
  justify-content: space-between;
  width: 100%;
  padding: 3.733333vw 2.666667vw 3.733333vw 0;
  align-content: center;
}
.address-index > i {
  font-size: 1.3rem;
  color: #ccc;
}
.btn-out {
  display: block;
  width: 100%;
  text-align: center;
  padding: 3.733333vw 0;
  margin: 5.333333vw 0;
  color: #ff5339;
  border-radius: 0.8vw;
  font-size: 1rem;
  font-weight: 700;
  background-color: #fff;
  border: 0;
}
</style>
